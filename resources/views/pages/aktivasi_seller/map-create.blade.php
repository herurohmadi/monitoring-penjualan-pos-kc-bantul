            ...
            <div class="card-body">
                <form class="row g-3" method="POST" action="{{ route('aktivasiseller.store') }}"
                    ...
                    <div class="col-12">
                        ...
                        <div id="locationMap" class="mt-2 border rounded"
                            style="display: none; width: 100%; height: 300px;" aria-label="Peta lokasi saat ini"></div>
...
@push('scripts')
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const address = document.getElementById('alamat_lengkap');
            let map = null;
            let marker = null;

            if (!button || !mapContainer || !navigator.geolocation) return;

            button.addEventListener('click', function() {
                button.disabled = true;
                button.innerHTML = '<i class="bx bx-loader-alt bx-spin me-1"></i> Mencari lokasi...';
                ...
                navigator.geolocation.getCurrentPosition(function(position) {
                    ...
                    if (marker) marker.setLatLng(coordinates);
                    else marker = L.marker(coordinates).addTo(map);
                    ...
                    fetch('https://nominatim.openstreetmap.org/reverse?format=jsonv2&lat=' +
                        ...
                        .then(data => {
                            if (data.display_name && address) address.value = data.display_name;
                        })
                        .catch(() => {});
                    ...
                    button.innerHTML =
                        '<i class="bx bx-current-location me-1"></i> Lokasi Saat Ini';
                    alert(
                        'Lokasi saat ini tidak dapat diperoleh. Pastikan izin lokasi telah diberikan.'
                        );
                });
            });
        });
